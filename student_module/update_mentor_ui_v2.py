...
target_file = r'd:\mentAi\src\pages\MentorMenteesPage.tsx'
...
DETAIL_FETCH_OLD = "if (d.success) setDetail(d.data); else toast.error(d.message);"
DETAIL_FETCH_NEW = "if (d.success) { setDetail(d.data); fetchMarks(studentId); } else toast.error(d.message);"
...
# Use a more stable anchor: find the useEffect and insert before it or its return
anchor = '    const info = detail;'
content = content.replace(anchor, DETAIL_FUNCS + "\n    " + anchor)
...
# Insert before "Contact Information" block
